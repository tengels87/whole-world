...
public class RPGCharacterController : MonoBehaviour {
    ...
    void Update() {
        ...
        bool inputJump = false;//Input.GetButtonDown("Jump");
        ...
        bool inputAttackL = false;//Input.GetButtonDown("AttackL");
        bool inputAttackR = false;//Input.GetButtonDown("AttackR");
        bool inputCastL = false;//Input.GetButtonDown("CastL");
        bool inputCastR = false;//Input.GetButtonDown("CastR");
        float inputSwitchUpDown = 0;//Input.GetAxisRaw("SwitchUpDown");
        float inputSwitchLeftRight = 0;//Input.GetAxisRaw("SwitchLeftRight");
        bool inputStrafe = false;//Input.GetKey(KeyCode.LeftShift);
        float inputTargetBlock = 0;//Input.GetAxisRaw("TargetBlock");
        ...
        bool inputAiming = false;//Input.GetButtonDown("Aiming");
    ...
    public void Blocking() {
        if (canAction && isGrounded) {//if (Input.GetAxisRaw("TargetBlock") < -0.1f && canAction && isGrounded) {
            if (!isBlocking) {
    ...
    public void Pickup() {
        ...
        StartCoroutine(_LockMovementAndAttack(0, 0.0f));// (0, 1.4f)
    }
